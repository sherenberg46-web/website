    ...
    <section>
      ...
      <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2 -mx-4 px-4 snap-x">
        {items.map((item) => (
          ...
          >
            <div className="relative aspect-[2/3] rounded-card overflow-hidden bg-surface-2 group-hover:shadow-card-hover transition-shadow duration-300">
              <FitImage
                src={item.image_url}
                alt={item.title}
                sizes="192px"
              ...
              />
              {item.discount_pct > 0 && (
                <span className="absolute top-1.5 left-1.5 px-1.5 py-0.5 bg-accent text-accent-contrast text-[10px] font-bold rounded-md">
