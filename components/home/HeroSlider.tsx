...
export function HeroSlider({ banners }: Props) {
  ...
  // Параллакс: фон уезжает медленнее, контент плавно тает при скролле
  const { scrollY } = useScroll();
  const bgY = useTransform(scrollY, [0, 800], [0, reduceMotion ? 0 : 160]);
  const contentOpacity = useTransform(scrollY, [0, 400], [1, reduceMotion ? 1 : 0]);
  const contentY = useTransform(scrollY, [0, 400], [0, reduceMotion ? 0 : 60]);
    ...
    >
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <AnimatePresence initial={false} mode="sync">
          ...
          >
            {imageFailed ? (
              /* Фолбэк: фирменный градиент + сетка вместо битой картинки */
              <div className="absolute inset-0">
                <div className="absolute inset-0 bg-gradient-to-br from-accent/20 via-bg-page to-accent-blue/20" />
                <div className="absolute inset-0 bg-[linear-gradient(rgba(0,212,170,0.04)_1px,transparent_1px),linear-gradient(90deg,rgba(0,212,170,0.04)_1px,transparent_1px)] bg-[size:60px_60px]" />
              </div>
            ) : (
              <Image
                src={imageUrl}
